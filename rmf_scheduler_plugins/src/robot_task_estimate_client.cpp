  ...
  const rmf_scheduler::task::EstimateRequest & request)
{
  ...
  request_publisher_->publish(
    ...
    .request_id(id)// Copyright 2023 ROS Industrial Consortium Asia Pacific
    //
    // Licensed under the Apache License, Version 2.0 (the "License");
    // you may not use this file except in compliance with the License.
    // You may obtain a copy of the License at
      ...
      const rmf_task_msgs::msg::ApiResponse & msg)
    {
      ...
      try {
        ...
        auto json_uri = nlohmann::json_uri{
          rmf_api_msgs::schemas::estimate_robot_task_response["$id"]
        };
      ...
      response.deployment_time =
        static_cast<uint64_t>(raw_response["result"]["deployment_time"].get<int>()) * 1e6;
      response.finish_time =
        static_cast<uint64_t>(raw_response["result"]["finish_time"].get<int>()) * 1e6;
      response.duration =
        static_cast<uint64_t>(raw_response["result"]["duration"].get<int>()) * 1e6;

      response.state.waypoint = raw_response["result"]["state"]["waypoint"].get<int>();
      response.state.orientation = raw_response["result"]["state"]["orientation"].get<double>();
      response.state.consumables["battery_soc"] =
        raw_response["result"]["state"]["battery_soc"].get<double>();
