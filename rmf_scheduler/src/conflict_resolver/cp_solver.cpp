...
public:
  ...
  struct EventCache
  {
    explicit EventCache(const Event & _event)
    {
      event = _event;
    }
   ...
   */
  EventVarPtr new_var(const Event & event);

  // Adds the difference in original start time and assigned start to objective function
  void add_abs_diff(const Event & event, const EventVarPtr event_var);
...
// Converts an Event into and EventVarPtr
CpSolver::Impl::EventVarPtr CpSolver::Impl::new_var(const Event & event)
{
  // Create variables
...
void CpSolver::Impl::add_abs_diff(
  const Event & event, const EventVarPtr event_var)
{
...
std::vector<EventChange> CpSolver::Impl::interpret_response(CpSolverResponse & response)
{
  ...
  {
    std::cout << "Optimal Re-Scheduling Amount: "
