...
int main(int argc, char * argv[])
{
  ...
  auto response_future_2 = client->async_send_request(estimate_request);
  auto response_future_3 = client->async_dispatch_request(dispatch_task_request);
  ...
  if (!response_future_3.valid()) {
    std::cout << "There is no future" << std::endl;
    return 1;
  }
  rclcpp::spin_until_future_complete(client->node(), response_future_3);
  auto response_3 = response_future_2.get();
  std::cout << "Got response: " << response_3.dump() << std::endl;
