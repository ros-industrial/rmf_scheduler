...
from rmf2_scheduler.data import Process, Graph, Edge, json_serializer
...
"""
    ...
    assert json_serializer.serialize(process.graph) == expected_graph_json
    expected_process_json = {"id": process.id, "graph": expected_graph_json}
    assert json_serializer.serialize(process) == expected_process_json

    duplicate_process = Process()
    json_serializer.deserialize(expected_process_json, duplicate_process)
